...
def mera_process(years):
    ...
    for var in var_dirs:
        ...
        file_list = list(chain(*list(
            ...
            for e in [
                f"*{i}*{var}_FC3hr*" for i in range(years[0], years[1] + 1)
            ]
        )))
        ...
        for f in file_list:
            ...
            # keep only the third forecast step, shift the time by -3 hours to
            # get the time at the start of the forecast, and convert to netCDF
            os.system(
            ...
            data = xr.open_dataset(
                f"{gf}.nc", decode_coords="all", chunks="auto"
            )
