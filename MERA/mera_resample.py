...
import xarray as xr
from data.MERA.mera_process import mera_process
...
def mera_resample(years):
    ...
    for var in var_dirs:
        # list of files for the given years
        file_list = list(chain(*list(
            glob.glob(os.path.join(DATA_DIR, f"{var}_FC3hr", e))
            for e in [
                f"*{i}*{var}_FC3hr.nc" for i in range(years[0], years[1] + 1)
            ]
        )))
        data = xr.open_mfdataset(
            file_list, chunks="auto", decode_coords="all"
        )
        ...
        # convert units
        if var in ("111_105_0_4", "112_105_0_4", "117_105_0_4"):
            data[varname] = data[varname] * (60 * 60 * 24 / 1e6)
        ...
        # save data
        data.to_netcdf(
            os.path.join(OUT_DIR, f"MERA_{years[0]}_{years[1]}_{var}_day.nc")
        )
